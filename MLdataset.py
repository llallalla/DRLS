...
class IncDataset(Dataset):
    def __init__(self,mat_path, fold_mat_path, training_ratio=0.7, val_ratio=0.15, fold_idx=0, mode='train',semisup=False):
        ...
        if mode=='train':
            self.M = torch.mm(torch.tensor(self.cur_labels.T, dtype=torch.float), torch.tensor(self.cur_labels, dtype=torch.float))
            self.N = torch.sum(torch.tensor(self.cur_labels, dtype=torch.float), dim=0)
